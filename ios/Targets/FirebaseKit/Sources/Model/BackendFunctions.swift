...
extension DB {
	...
	public struct ImageAnalysisResultType: Codable {
		...
		public let audio: String?  //base64 mp3 audio
	...
	public func processImageWithAI(jpegImageBase64: String, processingCommand: String, readResultOutLoud: Bool) async
		-> ImageAnalysisResultType?
	{
		Analytics.capture(
			.info, id: "process_image_with_ai_called",
			longDescription: "processingCommand: \(processingCommand), readResultOutLoud: \(readResultOutLoud)",
			source: .aikit)
		do {
			let contentDict: [String: Any] = [
				"imageBase64": jpegImageBase64, "processingCommand": processingCommand,
				"readOutLoud": readResultOutLoud,
			]

			let data = try await functions.httpsCallable("analyzeImageContents").call(contentDict)

			guard let processResultDataRaw = data.data as? [String: Any] else {
				Analytics.capture(
					.error, id: "process_image_with_ai",
					longDescription: "Data Mismatch: data is not an array.", source: .aikit)
				return nil
			}

			Analytics.capture(.success, id: "process_image_with_ai", source: .aikit)
			return try? ImageAnalysisResultType(from: processResultDataRaw)
		} catch {
			Analytics.capture(
				.error, id: "process_image_with_ai",
				longDescription: "Error processing image with AI: \(error.localizedDescription)", source: .aikit)
			return nil
		}
	}
